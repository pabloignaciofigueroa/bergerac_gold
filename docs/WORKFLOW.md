...
| `sin-webgl` | **Sin WebGL en absoluto, Bergerac sigue en pie.** Perfil distinto al de `resiliencia`: allí se tiran contextos ya creados, aquí WebGL no existe desde el primer momento —aceleración por hardware desactivada, GPU en lista negra, extensión que lo bloquea—. Comprueba que las seis secciones existen, que **los cuatro** instrumentos enseñan su estado fijo, que el título del hero se lee en tipografía real, que los vídeos de casos NO reciben el aviso de WebGL, que el scroll opera y que la consola queda limpia. Deja capturas en `qa-out/sin-webgl/`. Se lanza con `node tools/qa/sin-webgl.mjs`. Validada al revés: con el selector anterior acusa seis fallos. |
| `estilos` | **Bajar a REDUCED no mueve la composición.** Compara el mismo build en sus dos niveles —`?calidad=full` contra `?calidad=reduced`— elemento por elemento: mismo número, mismas cajas, mismos estilos computados. Lo único que puede diferir es la resolución del lienzo, y solo en CANVAS y solo si la PROPORCIÓN se mantiene. Aviso: ya no compara contra el sitio anterior a Astro; esa referencia dejó de arrancar cuando las dependencias vendorizadas pasaron a npm. |
| `fotogramas` | **La red visual está donde tiene que estar, y solo ahí.** Dos mitades: que aparezcan cuando toca —sin WebGL, contexto perdido, reduced-motion, o la escena que no llega a arrancar— y que **NO se descargue ni uno en FULL ni en REDUCED**. En el Método comprueba que los cuatro se turnan con las etapas y que la bisagra cromática los acompaña: un fotograma con luz fucsia sobre fondo morado sería peor que no tener fotograma. El caso "no arranca" se simula bloqueando el trozo de código de la escultura. `node tools/qa/fotogramas.mjs`. |
| `capturar-fotogramas.mjs` | Script aparte, no es una prueba: vuelve a sacar los fotogramas de las escenas reales. Pide los píxeles al lienzo con `toDataURL()` —de ahí la costura `?fotograma=1`, que enciende `preserveDrawingBuffer`— y las vistas del Método por su índice en `DEFAULT_VIEWS`, no adivinando posiciones de scroll. Deja también capturas en contexto para revisarlas. |
| `calidad.test.mjs` | **El decisor de calidad, en seco.** Sin navegador ni relojes: se le dan muestras de fotograma inventadas y se comprueba qué decide. Cubre el pico aislado que NO debe bajar, el calentamiento por tiempo, que una escena pesada no arrastre a una barata, y —por enumeración sobre 800 barridos— que desde REDUCED no existe ninguna entrada que suba el nivel. Corre en milisegundos: `node tools/qa/calidad.test.mjs`. |
| `calidad` | **FULL donde se puede, REDUCED donde hace falta.** Cuatro perfiles en Chrome real: `normal` (GPU de verdad → los cuatro instrumentos siguen en FULL, que es la garantía de que esto no degrada a quien no le hace falta), `software` (SwiftShader → Hero/Estudio/Método arrancan en REDUCED y **Contacto se queda en FULL**), `lento` (CPU 6×) y `forzado`. Se puede correr uno solo: `node tools/qa/calidad.mjs software`. |
| `calidad · perfil lento` | Aviso importante: **la CPU estrangulada prueba que el mecanismo de bajada funciona, no simula una GPU débil.** Estrangular la CPU no reduce el relleno, que es donde sufren estas escenas. Además deja a varias escenas rozando el techo, así que quién cruza cambia de una pasada a otra: por eso la puerta no exige “alguien tiene que bajar”, sino el CONTRATO —baja exactamente quien acumuló dos ventanas seguidas por encima de su techo, y nadie más—, que se cumple en cualquier hardware. |
| `calidad · perfil forzado` | Lo que sí demuestra el camino entero de forma determinista. Con `forzarPresupuesto(0)` la bajada ocurre siempre, y se mide el **lienzo real**: 2880 → 1440 px y 1670 → 835 px. Es la prueba de que la reducción llega a la GPU y no se queda en una etiqueta. |
| `calidad-visual.mjs` | Script aparte, no es una prueba: captura los cuatro instrumentos en FULL y en REDUCED para compararlos con el ojo. Lo mismo se ve en vivo con `?calidad=full` y `?calidad=reduced`. |
