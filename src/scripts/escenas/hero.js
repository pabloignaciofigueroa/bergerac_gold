...
export async function init(mount) {
  ...
  /* tinte reforzado (pedido QA): 50% más de hundido hacia grafito */
  const tono = new THREE.Color(PALETA.azul).lerp(new THREE.Color(PALETA.grafito), .3);
  ...
  const base = crearBase(mount, {
    ...
    onFrame(t, dt) {
      ...
      for (let i = 0; i < N; i++) {
        ...
        // velocidad al 50% de nuevo (pedido QA): mismas conductas, vuelo calmo
        const rapidez = v.length();
        const max = (enFuga ? 2.125 : 1.15) + estallidoFuerza, min = .45;
